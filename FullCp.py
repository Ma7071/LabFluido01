...
CHORD = 0.2                 # m, chord length (normalized)
...
for sensor in sensors:
    ...
    # Select proper slope based on sensor position (upper/lower surface)
...
for file in pressure_files:
    try:
        # Extract angle from the file name: for "-8.5_gradi.txt", this returns "-8.5"
        ...
        pressure_data_all[angle] = avg_pressures  # Store in dictionary
...
for angle in sorted(pressure_data_all.keys()):
    ...
    # Split pressures into two groups:
    # Sensors 1-8 (upper surface) and 9-16 (lower surface)
    pressures_upper = avg_pressures[:8]
    pressures_lower = avg_pressures[8:]

    # Use sensor x-positions for interpolation from the sensors array
    x_upper = sensors[:8, 1]
    x_lower = sensors[8:16, 1]

    # ---- Modified Cl Calculation ----
    # First, compute the sensor-based pressure coefficients at the sensor locations:
    q_inf = 0.5 * AIR_DENSITY * FREESTREAM_VELOCITY**2  # Dynamic pressure
    cp_up_sensor = pressures_upper / q_inf
    cp_low_sensor = pressures_lower / q_inf

    # Define the convergence Cp as the average of the trailing-edge Cp values for both surfaces
    cp_convergence = (cp_up_sensor[-1] + cp_low_sensor[-1]) / 2.0

    # Append the extra point at x/c = 1.1 to the sensor data arrays for both surfaces:
    ...
    # Create new cubic spline interpolators that now include the extra x/c point (extrapolation enabled)
    ...
    # Save these interpolators for later use (storing the original pressure values interpolators is optional)
    pressure_interpolators[angle] = {"upper": cs_upper, "lower": cs_lower}

    # Define the integration domain from x/c = 0 to 1
    x_integration = np.linspace(0, 1, 1000)
    cp_upper_eval = cs_upper(x_integration)
    cp_lower_eval = cs_lower(x_integration)

    # Compute Cl by integrating the difference (lower Cp - upper Cp) along the chord
    cl = integrate.simpson(cp_lower_eval - cp_upper_eval, x_integration)
    cl_values[angle] = cl

    # ---- Pressure Interpolation Plotting (using original sensor data for visual continuity) ----
    # Create a fine grid for each side's x-domain
    x_fine_upper = np.linspace(x_upper.min(), x_upper.max(), 1000)
    x_fine_lower = np.linspace(x_lower.min(), x_lower.max(), 1000)
    # Interpolate using the original sensor pressures for visual plots:
    ...
    # ---- Create a combined plot for the current angle ----
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 6))

    # Plot 1: Airfoil with sensors and normals
    ...
    for idx, (sensor, (x_sensor, normal_angle)) in enumerate(zip(sensors, sensor_angles)):
        ...
        ax1.plot(x_sensor_val, z_sensor, 'ko')  # Sensor point
    ...
    # Plot 2: Pressure interpolation curves and Cl display
    ...
    # Save combined plot for this angle in the output directory
    ...
    # Create a separate directory for Cp distribution plots
    cp_directory = "cp_plots"
    if not os.path.exists(cp_directory):
        os.makedirs(cp_directory)

    plt.figure(figsize=(8, 6))
    # For plotting the Cp distribution, evaluate the modified splines over the overlap domain (x/c=0 to 1)
    ...
    plt.gca().invert_yaxis()  # Invert y-axis (common for Cp plots)
    plt.legend()
...
# Create a cubic spline interpolation for Cl vs. Angle
cs_cl = CubicSpline(angles, cl_arr)

# Generate a fine grid of angles for a smooth interpolation curve
...
# === PART 2: Compute Profile Drag Coefficient (CD) from wake data ===


# Constants
rho = 1.174   # air density in kg/m³
V_inf = 20.0  # freestream velocity in m/s
d = 0.2       # model diameter (or chord) in meters

# Directory containing wake survey data files
data_dir = 'CDdata'
...
for fname in os.listdir(data_dir):
    if fname.endswith('.txt'):
        # Extract angle of attack from filename
        alpha = float(fname.replace('.txt', ''))
        alpha_list.append(alpha)

        # Load z and dynamic pressure from file
        z_mm, P_dyn = np.loadtxt(os.path.join(data_dir, fname), unpack=True)
        z = z_mm / 1000  # convert mm → m

        # Compute local velocity from dynamic pressure
        V = np.sqrt(2 * P_dyn / rho)

        # Compute wake momentum deficit integral
        integrand = (1 - V / V_inf) * V
        D_prime = rho * np.trapz(integrand, z)  # drag per unit span

        # Compute drag coefficient
        CD = D_prime / (0.5 * rho * V_inf**2 * d)
        CD_list.append(CD)
...
# Fit quadratic: CD = CD0 + k * alpha^2
...
# Plot drag polar
plt.figure()
plt.plot(alpha_array**2, CD_array, 'bo', label='Data')
plt.plot(alpha_array**2, CD0 + k * alpha_array**2, 'r--', label='Fit: CD = CD0 + k α²')
plt.xlabel("α² (deg²)")
plt.ylabel("C_D")
plt.title("Drag Polar from Wake Survey")
plt.legend()
plt.grid(True)
plt.show()

# === SAVE DRAG POLAR DATA & FITTED PARAMETERS ===

# Ensure output directory
output_dir = 'airfoil_plots'
os.makedirs(output_dir, exist_ok=True)

# 1) Save raw polar points to CSV
csv_path = os.path.join(output_dir, 'drag_polar.csv')
with open(csv_path, 'w') as f:
    f.write('alpha_deg,alpha2_deg2,C_D\n')
    for a, cd in zip(alpha_array, CD_array):
        f.write(f'{a:.3f},{a**2:.6f},{cd:.6f}\n')
print(f"Saved raw drag-polar data → {csv_path}")

# 2) Save fitted coefficients to text file
coeff_path = os.path.join(output_dir, 'drag_polar_coeffs.txt')
with open(coeff_path, 'w') as f:
    f.write(f'CD0: {CD0:.6f}\n')
    f.write(f'k:   {k:.6f}\n')
print(f"Saved fitted drag-polar coefficients → {coeff_path}")
