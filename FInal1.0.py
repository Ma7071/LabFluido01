...
for fname in os.listdir(data_dir):
    ...
    # 2) Load data: z[mm], q_inf[Pa], q[Pa]
    data = np.loadtxt(os.path.join(data_dir, fname), delimiter='\t', skiprows=1)
    z_mm = data[:, 0]
    q_inf_all = data[:, 1]
    q_loc_all = data[:, 2]

    # 3) Average q values at each unique z
    ...
    z_m = unique_z / 1000.0  # convert to meters

    # 4) Compute the integrand for momentum deficit method
    integrand = (np.sqrt(avg_q_loc / avg_q_inf) - (avg_q_loc / avg_q_inf)) * (z_m / d)

    # 5) Integrate using trapezoidal rule
    CD = 2 * np.trapezoid(integrand, z_m)
    CD_list.append(CD)
